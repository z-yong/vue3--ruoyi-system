...
export function isMobile() {
  ...
  return (/Android|webOS|iPhone|iPod|BlackBerry|SymbianOS|HarmonyOS|iPad/i.test(navigator.userAgent))
}
  ...
  if (sColor) {
    if (sColor.length === 4) {
      ...
      for (let index = 1; index < 4; index += 1) {
        sColorNew += sColor.slice(index, index + 1).concat(sColor.slice(index, index + 1));
      }
      sColor = sColorNew;
    }
    // 处理六位的颜色值
...
export function checkEmail() {
  return [{
    validator: (rule, value, callback) => {
      if (value !== '') {
        if (!(/^[a-zA-Z0-9_.-]+\@+[0-9a-zA-Z]+\.(com|com.cn|edu|hk|cn|net)$/.test(value))) {
  ...
  return [{
    validator: (rule, value, callback) => {
      if (value !== '') {
        ...
        } else if (value.toString().length > 20 || value.toString().length < 6) {
...
export function isEmail(value) {
  return !(/^[a-zA-Z0-9_.-]+\@+[0-9a-zA-Z]+\.(com|com.cn|edu|hk|cn|net)$/.test(value));
}
...
// 浮点数加法精度丢失问题
export function numAdds(arg1, arg2) { // 要相加的两个数
  let r1
  let r2
  let m
  try {
...
export function numAdd(arg1, arg2) { // 要相加的两个数
  let r1
  let r2
  let m
  try {
